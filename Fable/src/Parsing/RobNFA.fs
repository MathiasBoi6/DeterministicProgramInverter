                ...
                | Nonterminal name ->
                    let nonterminalState =  
                        ...
                        | Some (nt, state) -> 
                            ...
                            state
                        | _ -> 
                            match nonterminalTable.[1] with
                            | (nt, state) -> printfn "nt = %A; name = %A; eq = %A" nt name (nt = name)
                            printfn "NTABLE = %A" nonterminalTable.[1]
                            failwithf "Error in CreateNFA, Could not find nonterminal %A" name
